    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-sage-100 flex items-center justify-around px-2 pt-2 pb-3 z-30">
      {items.map(item => (
        <button key={item.href} onClick={() => router.push(item.href)}
          className={`flex flex-col items-center gap-0.5 flex-1 ${item.center ? '-mt-6' : ''}`}>
          {item.center ? (
            <span className="w-12 h-12 rounded-full bg-gold-400 text-white flex items-center justify-center text-2xl shadow-lg">+</span>
          ) : (
